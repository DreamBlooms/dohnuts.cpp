...
struct http_server::impl {
    ...
    impl(http_options value, predictor callback) : options(std::move(value)), predict(std::move(callback)) {
        ...
        // cpp-httplib ThreadPool is (base_threads, max_threads, max_queued_requests).
        // Keep the pool small: inference is serialized and compute threads need the cores.
        const size_t base_workers = 4;
        const size_t max_workers = std::min<size_t>(16, options.max_pending_requests + 8);
        server.new_task_queue = [base_workers, max_workers] {
            return new httplib::ThreadPool(base_workers, max_workers, 32);
        };
        ...
        server.Get("/health", [this](const auto&, auto& response) {
            ...
            send(response, 200, {{"status", "ok"}, {"model", model},
        ...
        server.Get("/v1/models", [this](const auto&, auto& response) {
            send(response, 200, {{"models", json::array({{{"name", model},
                {"description", "Dohnuts native typed decisions on llama.cpp"},
    ...
    void validate(const json& request, size_t& questions) const {
        ...
        for (const auto& definition : definitions) {
            const auto instruction = definition.value("instructions", json());
            if (!instruction.is_null() && !instruction.is_string() && !instruction.is_structured())
    ...
    void evaluate(const httplib::Request& request, httplib::Response& response, bool batch_route) {
        try {
            ...
            response.set_header("X-Dohnuts-Batch-Id", std::to_string(completed.batch));
            response.set_header("X-Dohnuts-Batch-Offset", std::to_string(completed.offset));
            if (batch_route) {
